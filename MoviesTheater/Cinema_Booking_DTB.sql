...
-- Run this on your SQL Server database (CinemaBookingDB or CinemaBookingDB_Test)

-- ResetToken columns (if not already added)
IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('Account') AND name = 'ResetToken')
    ALTER TABLE Account ADD ResetToken VARCHAR(255) NULL;
IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('Account') AND name = 'ResetTokenExpiry')
    ALTER TABLE Account ADD ResetTokenExpiry DATETIME NULL;

-- DeletionRequest table
IF NOT EXISTS (SELECT * FROM sys.objects WHERE name = 'DeletionRequest' AND type = 'U')
CREATE TABLE DeletionRequest (
    ...
    CONSTRAINT FK_DR_Account FOREIGN KEY (AccountID) REFERENCES Account(AccountID),
    CONSTRAINT FK_DR_ReviewedBy FOREIGN KEY (ReviewedBy) REFERENCES Account(AccountID),
