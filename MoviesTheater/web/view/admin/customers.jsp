...
<head>
    ...
    <title>Quản lý khách hàng — CGV System Admin</title>
    ...
    <style>
        ...
        .pagination a { padding:8px 14px; border:1px solid var(--cgv-border); border-radius:6px; text-decoration:none; color:var(--cgv-text); font-size:14px; }
        .pagination a.active { background:var(--cgv-red); color:#fff; border-color:var(--cgv-red); }
    </style>
</head>
<body class="cgv-body">
...
<div class="cgv-main">
    <header class="cgv-header">
        <h1 class="cgv-header-title">Quản lý khách hàng</h1>
    ...
    <div class="cgv-page">
        <div class="cgv-list-wrap">
            ...
            <div class="toolbar">
                <form method="get" action="${pageContext.request.contextPath}/admin/customers" style="display:contents;">
                ...
                <a href="${pageContext.request.contextPath}/admin/customers?status=blocked${not empty param.q ? '&q='.concat(param.q) : ''}${not empty param.sortBy ? '&sortBy='.concat(param.sortBy).concat('&sortOrder=').concat(param.sortOrder) : ''}"
                   class="toolbar-btn ${param.status eq 'blocked' ? 'active' : ''}">Đã khoá</a>
                <a href="${pageContext.request.contextPath}/admin/customers?status=active${not empty param.q ? '&q='.concat(param.q) : ''}${not empty param.sortBy ? '&sortBy='.concat(param.sortBy).concat('&sortOrder=').concat(param.sortOrder) : ''}"
                   class="toolbar-btn ${param.status eq 'active' ? 'active' : ''}">Hoạt động</a>
                <a href="${pageContext.request.contextPath}/admin/customers" class="toolbar-btn">Tất cả</a>

                <form method="post" action="${pageContext.request.contextPath}/admin/customers" id="exportCsvForm" style="display:inline;">
            ...
            <div class="batch-bar" id="batchBar">
                ...
                <span class="count" id="selectedCount">0</span> khách hàng được chọn
                <form method="post" action="${pageContext.request.contextPath}/admin/customers" id="batchForm" style="display:inline;">
                ...
                <tbody>
                    ...
                    <tr style="${u.isBlocked ? 'opacity:0.6;background:#fef2f2;' : ''}"
                        ...
                        <td style="display:flex;gap:6px;flex-wrap:wrap;">
                            <button type="button" class="action-btn view" onclick="viewCustomer(this)">Xem</button>
                    ...
                    <tr><td colspan="9" style="padding:40px;text-align:center;color:var(--cgv-text-muted);">Không có khách hàng nào.</td></tr>
                    </c:if>
                </tbody>
            </table>
